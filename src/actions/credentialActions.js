...
export const addCredential = (bookmaker, country, bookmakerUsername, password, currency) => async (dispatch) => {
  ...
  await axios.post(`${constant.API_URL}/credential`, data, {
    ...
    .then((res) => {
      if (res.status === 200) {
        const message = `${userData.surname} ${userData.firstname} added a new bookieaccount - "${res.data.bookmakerUsername}" on ${res.data.bookmaker}`;
...
export const updateCredential = (bookmaker, country, bookmakerUsername, password, currency, id) => async (dispatch) => {
  ...
  await axios.put(`${constant.API_URL}/credential/${id}`, data, {
    ...
    .then((res) => {
      if (res.status === 200) {
        const message = `${userData.surname} ${userData.firstname} updated bookieaccount - "${res.data.bookmakerUsername}" on ${res.data.bookmaker}`;
        window.$client.updateBookieaccount({ userId: userData.id, message });
        dispatch({
...
export const deleteCredential = (id) => async (dispatch) => {
  ...
  await axios.delete(`${constant.API_URL}/credential/${id}`, {
    ...
    .then((res) => {
      if (res.status === 200) {
        const message = `${userData.surname} ${userData.firstname} deleted bookieaccount - "${res.data.bookmakerUsername}" on ${res.data.bookmaker}`;
        window.$client.updateBookieaccount({ userId: userData.id, message });
        dispatch({
