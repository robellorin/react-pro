...
const monday = moment().isoWeekday(1).utc().format('DD');
const today = moment().utc().format('DD');


function Overview({
  className, weekData, month, currency = 'EUR', ...rest
}) {
  const classes = useStyles();

  // if (!weekData.pl) return null;
  const data = {
    pl: weekData.pl,
    rollover: weekData.rollover,
    roi:
      weekData.rollover > 0 ? Math.round((weekData.pl / weekData.rollover) * 10000)
      / 100 : 0
  };
  const stateList = [
    {
      title: 'Period',
      value: `Week / ${monday} - ${today}`,
      icon: TodayIcon,
